...
function Story() {
  ...
  if (!data) {
    return (
      <div className="mx-auto max-w-3xl px-6 py-24 md:py-40 text-center">
        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground mb-6">{T.journal.comingSoon}</p>
        <h1 className="font-display text-6xl md:text-7xl font-light tracking-[-0.04em] capitalize">{slug.replace(/-/g, " ")}</h1>
        <p className="mt-8 text-muted-foreground">
          {T.journal.notPublished} <Link to="/journal" className="link-underline">{T.journal.backTo}</Link>.
        </p>
      </div>
    );
  }
  ...
  return (
    <article className="mx-auto max-w-3xl px-6 py-16 md:py-24">
      <Link to="/journal" className="text-xs uppercase tracking-[0.2em] text-muted-foreground link-underline">{T.journal.back}</Link>
